import pygame
import time
from pygame.surface import Surface
from pygame.font import Font
from pygame.event import Event


def main() -> None:
    pygame.init()  # Prepare the PyGame module for use
    main_surface: Surface = pygame.display.set_mode((480, 240))

    # Load an image to draw. Substitute your own.
    # PyGame handles gif, jpg, png, etc. image types.
    ball: Surface = pygame.image.load("ball.png")

    # Create a font for rendering text
    my_font: Font = pygame.font.SysFont("Courier", 16)

    frame_count: int = 0
    frame_rate: float = 0
    t0: float = time.process_time()

    while True:
        # Look for an event from keyboard, mouse, joystick, etc.
        ev: Event = pygame.event.poll()
        if ev.type == pygame.QUIT:  # Window close button clicked?
            break  # Leave game loop

        # Do other bits of logic for the game here
        frame_count += 1
        if frame_count % 500 == 0:
            t1: float = time.process_time()
            frame_rate = 500 / (t1 - t0)
            t0 = t1

        # Completely redraw the surface, starting with background
        main_surface.fill((0, 200, 255))

        # Put a red rectangle somewhere on the surface
        main_surface.fill((255, 0, 0), (300, 100, 150, 90))

        # Copy our image to the surface, at this (x,y) posn
        main_surface.blit(ball, (50, 70))

        # Make a new surface with an image of the text
        the_text: Surface = my_font.render(
            "Frame = {0}, rate = {1:.2f} fps".format(frame_count, frame_rate),
            True,
            (0, 0, 0),
        )

        # Copy the text surface to the main surface
        main_surface.blit(the_text, (10, 10))

        # Now that everything is drawn, put it on display!
        pygame.display.flip()
    pygame.quit()


if __name__ == "__main__":
    main()
